...
const dataTypes = {
    ...
    phone: {
        icon: 'fas fa-phone text-green-600',
        iconFallback: '📞',
        label: 'Phone',
        enabled: true,
    },
    ...
    discord: {
        icon: 'fab fa-discord text-indigo-600',
        iconFallback: '🎮',
        label: 'Discord',
        enabled: true,
    },
    ...
    tiktok: {
        icon: 'fab fa-tiktok text-gray-800',
        iconFallback: '🎵',
        label: 'TikTok',
        enabled: true,
    },
    spotify: {
        icon: 'fab fa-spotify text-green-600',
        iconFallback: '🎧',
        label: 'Spotify',
        enabled: true,
    },
    ...
    file: {
        icon: 'fas fa-file text-purple-600',
        iconFallback: '📄',
        label: 'File',
        enabled: false,
    },
    address: {
        icon: 'fas fa-map-marker-alt text-red-600',
        iconFallback: '📍',
        label: 'Address',
        enabled: false,
    },
...
const validators = {
    ...
    instagram: (value) => {
        const patterns = [/instagram\.com\/([a-zA-Z0-9_.]+)/, /^@?([a-zA-Z0-9_.]+)$/];
    ...
    telegram: (value) => {
        const patterns = [/t\.me\/([a-zA-Z0-9_]+)/, /^@?([a-zA-Z0-9_]+)$/];
        ...
        logoFileInput.addEventListener('change', async function (e) {
            ...
            try {
                ...
                if (preview) {
                    preview.classList.remove('hidden');
                    preview.innerHTML =
                        '<p class="text-xs text-gray-500 dark:text-gray-400 mb-2">✓ Ảnh đã được crop vuông:</p>';
                    preview.appendChild(img);
                }

                ActivityLogger.log('Logo uploaded and cropped', {
                    fileName: file.name,
                    fileSize: file.size,
                });
...
function updateFields() {
    ...
    typeFields.forEach((field) => {
        ...
        input.className =
            'w-full px-4 py-3 border-2 border-gray-200 dark:border-gray-600 rounded-xl focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 transition-all input-bg';
    ...
    if (validator && data.url) {
        ...
        if (!result.valid) {
            const errorMsg = result.messageKey
                ? LanguageManager.translate(result.messageKey)
                : 'Invalid input';
            alert(errorMsg);
            return null;
        }
